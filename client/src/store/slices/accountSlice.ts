...
const accountSlice = createSlice({
  name:'account', //Redux DevTools에서 구분할 때 사용
  initialState,
  reducers:{
    //* 하나의 액션으로 모든 필드 (id, email 등)를 갱신할 수 있게 만든 재사용 가능한 액션
    //* keyof AccountState -> 타입 안전하게 접근되도록 구성
    setField:(state, action:PayloadAction<{field: keyof AccountState; value:string}>)=>{
...
export const { setField, resetAccount } = accountSlice.actions;
