...
export function getHoidingWithAvg(items:TradeItem[]):HoldingItem[]{
  ...
  for(const item of items){
    const record = map.get(item.stock_name) || {
      quantity: 0,
      totalBuy: 0,
      buyAmount: 0
    }
    ...
    map.set(item.stock_name, record)
  }

  const result: HoldingItem[]=[]

  for(const [stock_name, data] of map.entries()){
    // 보유수량 >0 인 종목만 결과에 포함
    if(data.quantity>0){
      result.push({
        stock_name,
        ...
        currentPrice: currentPrices[stock_name] || 0 // 현재가 객체에서 조회 
      })
    }
  }
  return result
