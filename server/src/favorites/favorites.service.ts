...
export class FavoritesService {
  ...
  async favorites(dto:FavoritesDto, req:FavoritesDto) {
    ...
    const query = `
    INSERT INTO favorites (user_id, symbol) VALUES($1, $2)
    `;
    ...
    if(check.rows.length === 0) {
      ...
      console.log(`${dto.symbol} 관심종목 등록 완료`, result.rows[0])
      return {
        message: `${dto.symbol} 관심종목 등록 완료`
      };
    } else {
      console.log(`${dto.symbol} 은 이미 등록된 관심 종목입니다.`)
      return {message: '이미 등록된 관심 종목 입니다.'}
    }
  }
}