...
// * UserService 생성자 생성.
export class FavoritesCheckService {
  ...
  async check(dto:FavoritesDto, req:FavoritesDto) {
    ...
    if(check.rows.length === 0) {
      console.log(`${dto.symbol}이 관심종목에 등록되어 있지 않습니다.`)
      return {
        message: `${dto.symbol}이 관심종목에 등록되어 있지 않습니다.`,
        status: false
    }
    } else {
      console.log(`${dto.symbol}이 관심종목에 되어 있습니다.`)
      return {
        message: `${dto.symbol} 관심종목 등록 됨.`,
        status: true
      }
    }
  }
